...
class AppStatsDataViewModel: ObservableObject {
    ...
    private init() {
        ...
        if let data = UserDefaults.standard.data(forKey: appStatsDataKey),
           let loadedStats = try? JSONDecoder().decode(AppStatsData.self, from: data) {
    ...
    private func checkAndResetDailyStats() {
        ...
        // 获取上次更新日期的日期部分
        let lastUpdateDay = calendar.startOfDay(for: appStatsData.lastUpdateDate)
        ...
        if lastUpdateDay != todayDay {
            ...
            if let yesterday = calendar.date(byAdding: .day, value: -1, to: todayDay),
               calendar.isDate(lastUpdateDay, inSameDayAs: yesterday) {
                ...
                if appStatsData.todayCompletedPomodoros > 0 {
                    ...
                    appStatsData.longestStreak = max(appStatsData.longestStreak, appStatsData.currentStreak)
                }
            ...
            if appStatsData.todayCompletedPomodoros > 0 {
                ...
                let dateKey = dateFormatter.string(from: appStatsData.lastUpdateDate)
                appStatsData.dailyPomodoros[dateKey] = appStatsData.todayCompletedPomodoros
            }
